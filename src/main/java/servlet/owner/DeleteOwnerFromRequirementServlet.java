 ...
 */
public class DeleteOwnerFromRequirementServlet extends HttpServlet {
    @Override
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            DbCon.mongodb.getCollection(Config.getRequirementCollection()).findOneAndUpdate(
                    ...
                    new Document("$pull", new Document("owner", owner.getId()))
            );

            DbCon.mongodb.getCollection(Config.getOwnerCollection()).findOneAndUpdate(
                    new Document("oid", owner.getId()),
                    new Document("$pull", new Document("requirement", requirement.getRid()))
            );
        }
        catch (Exception e) {
